...
fn display(rr: &ResourceRecord, fmt: &str, raw_ttl: bool, name_length: usize, puny: bool) {
    for f in fmt.split(",") {
        match f.trim() {
            ...
            "type" => print!("{:<TYPE_DISPLAY_LENGTH$} ", rr.r#type),
            ...
            "ttl" => {
                if let Some(r) = rr.opt_or_class_ttl.regular() {
                    if raw_ttl {
                        ...
                        print!("{:<TTL_STRING_DISPLAY_LENGTH$} ", Ttl::from(r.ttl()))
                    }
                }
            }
            "rdata" => print!("{}", rr.r_data),
